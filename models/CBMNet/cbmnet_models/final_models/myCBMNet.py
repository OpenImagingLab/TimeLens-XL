...
from cbmnet_models.final_models.submodules import *
import numpy as np
import torch
...
try:
    ...
    # pass
    print("use self-implemented CV")
    from .newcorrelation import Correlation
...
def resize_flow(flow, width_im, height_im):
    flow_rescale = rescale_flow(flow, width_im, height_im)
    flow_up = upsample2d_hw(flow_rescale, height_im, width_im)
    return flow_up
...
class BackwardWarp(nn.Module):
	...
	def forward(self, img, flow):
		...
		x = gridX.to(u.device) + u
		y = gridY.to(v.device) + v
		# range -1 to 1
		x = 2 * (x / W - 0.5)
		y = 2 * (y / H - 0.5)
		# stacking X and Y
...
class FlowNet(nn.Module):
    ...
    def forward(self, batch, with_im=False):
        # F for frame feature
        # E for event feature
        ### encoding
        ## image feature
        ...
        for level, (E_t0_flow, E_t1_flow, E_0t, E_t1, F0, F1) in enumerate(zip(E_t0_pyramid_flow, E_t1_pyramid_flow, E_0t_pyramid, E_t1_pyramid, F0_pyramid, F1_pyramid)):
            if level==0:
                ...
                ## event flow generation]
                upflow_t0 = resize_flow(flow_t0_out_dict[level - 1], E_t0_flow.size(3), E_t0_flow.size(2))
                upflow_t1 = resize_flow(flow_t1_out_dict[level - 1], E_t0_flow.size(3), E_t0_flow.size(2))
                ...
                # decoder event
                flow_t0_ev_up = resize_flow(flow_event_t0, E_t0_flow.size(3), E_t0_flow.size(2))
                flow_t1_ev_up = resize_flow(flow_event_t1, E_t0_flow.size(3), E_t0_flow.size(2))
                ...
                # flow rescale
                down_evflow_t0 = resize_flow(flow_event_t0, F0.size(3), F0.size(2))
                down_evflow_t1 = resize_flow(flow_event_t1, F0.size(3), F0.size(2))
                down_upflow_t0 = resize_flow(flow_t0_out_dict[level-1], F0.size(3), F0.size(2))
                down_upflow_t1 = resize_flow(flow_t1_out_dict[level-1], F0.size(3), F0.size(2))
            ...
            # flow rescale
            down_flow_fusion_t0 = resize_flow(flow_fusion_t0, F0.size(3), F0.size(2))
            down_flow_fusion_t1 = resize_flow(flow_fusion_t1, F0.size(3), F0.size(2))
            ...
            upflow_t0_frame = resize_flow(flow_t0_frame, flow_fusion_t0.size(3), flow_fusion_t0.size(2))
            upflow_t1_frame = resize_flow(flow_t1_frame, flow_fusion_t1.size(3), flow_fusion_t1.size(2))
        ...
        i0tf = self.warp(batch['image_input0'], flow_t0_dict[0])
        i1tf = self.warp(batch['image_input1'], flow_t1_dict[0])
        ...
        if self.tb_debug:
            ...
            return flow_t0_dict, flow_t1_dict, i0tf, i1tf
...
class EventInterpNet(nn.Module):
    def __init__(self, params, num_bins=16, flow_debug=False):
        ...
        if self.params.training_stage == 'joint':
    ...
    def load_pretrain_network(self):
        ...
        weights = torch.load(self.params.pretrain_weights_dict['FlowNet'])['model_state']
        flownet_weights = OrderedDict()
        for k in self.flownet.state_dict():
            flownet_weights.update({k:weights[f"net.flownet.{k}"]})
        self.flownet.load_state_dict(flownet_weights)
        for j, p in enumerate(self.flownet.parameters()):
            p.requires_grad_(False)
        print("/n/n/n/n CBMNet flow network weights loaded. Paramters are fixed")
        return
    ...
    def forward(self, batch, mode='joint'):
        if mode=='flow':
            return self.flownet(batch, with_im=True)
        elif mode=='joint' or 'tuning':
