...
class FinalBidirectionAttenfusion(nn.Module):
	...
	def __init__(self, **kwargs):
		...
		## event
		self.events_encoder = EventsEncoder(self.echannel, self.img_chn, self.base_channel, num_decoder=self.num_decoder)
	...
	def forward(self, x, event, interp_ratio, end_time=None):
		...
		forwardflowlist, backwardflowlist = [], []
		if end_time == range(t-1):
			for t_ in range(t-1):
				...
				# fuseout.append(fo)
				forwardflowlist.append(m)
				backwardflowlist.append(fo)
		else:
			for t_ in end_time:
				res, m, fo = self.masknet(im0,
				                      im1,
		...
		return torch.stack(resout, 1), torch.stack(forwardflowlist), torch.stack(backwardflowlist)
