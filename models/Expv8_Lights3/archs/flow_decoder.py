...
class ResBlock(nn.Module):
	def __init__(self, in_channel, base_channel):
		...
		self.conv_in = nn.Sequential(
			nn.Conv2d(in_channel, base_channel, 1, 1, 0, bias=True),
		)
...
class IFBlock(nn.Module):
	def __init__(self, in_channel, base_channel):
		...
		self.resConv = ResBlock(in_channel, base_channel)
...
class MaskNet(nn.Module):
	def __init__(self, in_channel, base_channel):
		...
		self.refine_blocks = ResBlock(3+64, 32)
		self.conv_out = nn.Conv2d(32, 3, 1, 1, bias=True)

	def forward(self, im0, im1, i0tf, i1tf, t, f_t0, f_t1, forward_feat, backward_feat):
		...
		refine_in = torch.cat((forward_feat_hd, backward_feat_hd, fuse_out), 1)
