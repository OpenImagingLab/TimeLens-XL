import sys
import os
sys.path.append(os.path.join(os.getcwd(), "models/timelens_flow/"))
from models.BaseModel import  BaseModel
import numpy as np
import torch
from tools.registery import MODEL_REGISTRY
from models.timelens_flow.timelens import attention_average_network
...
class TimeLens_flow(BaseModel):
    ...
    def forward(self, pack_data):
        if self.training_stage == 'warp':
            self.net(pack_data)
            res = pack_data['middle']['flow_res']
            return res, res, res
        return self.net(pack_data)
