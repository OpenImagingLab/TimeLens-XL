...
from models.REFID.archs.recurrent_sub_modules import ConvLayer, UpsampleConvLayer, \
	ResidualBlock, ImageEncoderConvBlock, \
...
class FinalDecoderRecurrentUNet(nn.Module):
	def __init__(self, img_chn, ev_chn, out_chn=3, skip_type='sum', activation='sigmoid',
	             num_encoders=3, base_num_channels=32, num_residual_blocks=2, norm=None,
	...
	def build_decoders(self):
		...
		for input_size in decoder_input_sizes:
			self.decoders.append(self.UpsampleLayer(input_size if self.skip_type == 'sum' else 2 * input_size,
			                                        ...
			                                        norm=self.norm))  # kernei_size= 5, padidng =2 before
...
class FinalBidirectionAttenfusion(FinalDecoderRecurrentUNet):
	...
	def __init__(self, img_chn, ev_chn, out_chn=3, skip_type='sum',
		...
		for input_size, output_size, encoder_index in zip(self.encoder_input_sizes, self.encoder_output_sizes,
		                                                  self.encoder_indexs):
