...
class FinalBidirectionAttenfusion(nn.Module):
	...
	def __init__(self, **kwargs):
		super().__init__()
		self.interp_ratio = kwargs['interp_ratio']
		...
		## event
		self.events_encoder = EventsEncoder(self.interp_ratio, self.echannel, self.img_chn, self.base_channel)
		self.im_encoder = ImageEncoder(self.img_chn//2, self.base_channel)
		try:
		# if True:
			weights = torch.load('./RGB_resOut_REFID/GOPRO_Expv3/weights/Expv3_10.pt')['model_state']
			newdict = OrderedDict()
			for k in self.encoder.state_dict().keys():
				newdict.update({k:weights[f'net.encoder.{k}']})
			self.encoder.load_state_dict(newdict)
			print('Successfully load init')
		except:
			print('Initial weights loading failed')
			pass
		self.build_decoders()
	...
	def forward(self, x, event, end_time=None):
		"""
		...
		e_out, e_out_large = self.events_encoder(im0, im1, event)
		im0_feat, im1_feat = self.im_encoder(x)
		...
		end_time = range(t) if end_time is None else end_time
		for t_ in range(max(end_time)+1):
			forward_cur_feat, forward_cur_flow, forward_cur_flow_large, forward_econv = self.decoder_0(e_out[:, t_], im0_feat,
			                                                    forward_cur_flow,
			                                                    forward_cur_feat,
			                                                    forward_econv,)
			backward_cur_feat, backward_cur_flow, backward_cur_flow_large, backward_econv = self.decoder_1(e_out[:, t-t_-1],
			                                                      im1_feat,
			                                                      backward_cur_flow,
			                                                      backward_cur_feat,
																backward_econv)
		...
		if end_time == range(t):
			for t_ in range(t-1):
				res, m, fo = self.masknet(im0,
				                      ...
				                      forward_ims[t_],
				                      backward_ims[t_],
				                      (self.interp_ratio-t_-1)/self.interp_ratio*tmask,
		...
		return torch.stack(resout, 1), torch.stack(forward_ims, 1), torch.stack(backward_ims, 1), \
			torch.stack(forward_Delta_flow_list, 1), torch.stack(backward_Delta_flow_list, 1), torch.stack(fuseout, 1)    # b,t,c,h,w
