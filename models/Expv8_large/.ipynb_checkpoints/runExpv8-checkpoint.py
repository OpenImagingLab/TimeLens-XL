...
class Expv8(BaseModel):
    ...
    def forward(self, left_frame, right_frame, events):
        res = self.net(torch.cat((left_frame, right_frame), 1), events)[0]
        jump_ratio = self.interp_ratio//self.real_interp
        return res[:, (jump_ratio-1)::jump_ratio] if jump_ratio > 1 else res
