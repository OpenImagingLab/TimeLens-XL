...
def trainREFID_REFID_largeMotion(args):
    paths = ED()
    ...
    paths.save = ED()
    paths.save.save_path = './RGB_resOut_REFID_largeMotion'
    paths.save.exp_path = os.path.join(paths.save.save_path, 'GOPRO'+f"_{args.model_name}" + f"{args.extension}")
    ...
    training_config.crop_size = 480 if hostname == 'server' else 96
    training_config.num_workers = 8 if hostname == 'server' else 4
    ...
    training_config.rgb_sampling_ratio = 16
    ...
    training_config.optim.scheduler_params.T_max = 140000
    ...
    validation_config.rgb_sampling_ratio = 16
