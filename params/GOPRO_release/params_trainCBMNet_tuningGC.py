...
def GOPRO_release_CBMNet_tuningGC(args):
    paths = ED()
    paths.train_rgb = GOPRO.train.dense_rgb
    paths.train_evs = GOPRO.train.evs_refid if hostname != 'local' else GOPRO.train.evs
    ...
    paths.test_evs = GOPRO.test.evs_refid if hostname != 'local' else GOPRO.test.evs
    # paths.test_path_dict = parse_path_common(paths.test_rgb, paths.test_evs)
    # Interpolation ratio to generate events
    # paths.test_rgb_skip = 8

    paths.save = ED()
    paths.save.save_path = './RGB_GOPRO_release'
    paths.save.exp_path = os.path.join(paths.save.save_path, 'GOPRO'+f"_{args.model_name}_tuningGC" + f"{args.extension}")
    ...
    training_config.num_workers = 24 if hostname == 'server' else 1
    training_config.batch_size = 6 if hostname == 'server' else 2
    ...
    training_config.optim.scheduler_params.milestones = [8, 12]
    ...
    training_config.losses.GroupCharbonier = ED()
    training_config.losses.GroupCharbonier.weight = [1., 0.1, 0.1]
    training_config.losses.GroupCharbonier.as_loss = True
    
    # training_config.losses.SmoothLoss = ED()
    # training_config.losses.SmoothLoss.weight = 10.
    # training_config.losses.SmoothLoss.as_loss = True
    ...
    training_config.train_stats.save_im_ep = 20
    ...
    validation_config.weights_save_freq = 2
    ...
    validation_config.num_workers = 4 if hostname == 'server' else 1
