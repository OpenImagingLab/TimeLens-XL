...
class loader_ERFNetv0Mix_weighted(Dataset):
    def __init__(self, para, training=True):
        ...
        self.interp_ratio_list = [para[self.key]['interp_ratio']] if self.key == 'validation_config' else [2, 4, 8, 16, 32]
        interp_ratio = []
        for irl in self.interp_ratio_list:
            for irl_ in range(irl-1):
                interp_ratio.append(irl)
        self.interp_ratio =  [para[self.key]['interp_ratio']] if self.key == 'validation_config' else interp_ratio
