...
class loader_timelens(BaseLoader):
    ...
    def events_dense2bins(self, events, sample_t):
        ...
        right_events = []
        for st in sample_t:
            left_event, ori_left_event = self.generate_left_events(events[:st*self.e_sampler])
            left_events.append(left_event)
            ori_left_events.append(ori_left_event)
            right_events.append(self.generate_right_events(events[st*self.e_sampler:]))
