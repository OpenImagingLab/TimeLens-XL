...
class loader_bsergb_augment_half(BaseLoader):
    ...
    def events_reader_aug(self, evs_data, h, w, hs, ws, temporal_flip):
        # evs_data = [np.load(ep) for ep in events_path]
        evs_voxels = []
        for ed in evs_data:
            exs = np.float32(ed['x'])
            eys = np.float32(ed['y'])
            eps = np.float32(ed['polarity'])
            ets = np.float32(ed['timestamp'])
            if temporal_flip:
                exs = exs[::-1]
                eys = eys[::-1]
                eps = 1-eps[::-1]
            evs_voxels.append(sample_events_to_grid(self.sub_div, h, w,exs, eys, ets, eps, hs, ws))
    ...
    def data_loading(self, paths, sample_t):
        ...
        evs_data = [np.load(ep) for ep in evs_sample]
        temporal_flip = (random.random() > 0.5) & self.augment
        # temporal flip
        if temporal_flip:
            evs_sample = evs_sample[::-1]
            rgb_sample = rgb_sample[::-1]
            rgb_name = rgb_name[::-1]
            # evs_data = [ep[::-1] for ep in evs_sample]
            # evs_data['x'] = evs_data['x'][::-1]
            # evs_data['y'] = evs_data['y'][::-1]
            # evs_data['timestamp'] = evs_data['timestamp'][::-1]
            # evs_data['polarity'] = 1-evs_data['polarity'][::-1]
        im0 = self.imreader_half(rgb_sample[0])
        im1 = self.imreader_half(rgb_sample[-1])
        h, w = im0.shape[1:]
        events = self.events_reader_aug(evs_data, h, w, 0, 0, temporal_flip)
        gts = [self.imreader_half(rgb_sample[st]) for st in sample_t]
        # random flip
        if self.augment:
            randflag = random.randint(0, 4)
            if randflag > 0:
                if randflag < 3:
                    im0 = torch.flip(im0, (randflag,))
                    im1= torch.flip(im1, (randflag,))
                    gts = [torch.flip(gt, (randflag,)) for gt in gts]
                    events = torch.flip(events, (randflag,))
                else:
                    im0 = torch.flip(im0, (1, 2))
                    im1= torch.flip(im1, (1, 2))
                    gts = [torch.flip(gt, (1, 2)) for gt in gts]
                    events = torch.flip(events, (1, 2))
    ...
    def __getitem__(self, item):
        ...
        h, w = im0.shape[1:]
        if self.crop_size:
            hs, ws = random.randint(10, h - self.crop_size-10), random.randint(10, w - self.crop_size-10)
